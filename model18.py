...
class Res_Block(tf.keras.Model):
    def __init__(self, channel_in = 64, channel_out = 64):
        ...
        self.conv1 = Conv2D(channel, kernel_size=(3,3), strides=1, padding='same', use_bias=False)
...
class ResNet(tf.keras.Model):
    def __init__(self, input_shape, output_dim):
        ...
        self.blocks = [
                ...
                Res_Block(64, 64),#6x18x256
                [Res_Block(64, 64) for _ in range(2)],#6x18x256
                Conv2D(128, kernel_size=1, strides=2),#3x9x512
                [Res_Block(128, 128) for _ in range(2)],#3x9x512
                Conv2D(256, kernel_size=1, strides=2, use_bias=False),#2x5x1024
                [Res_Block(256, 256) for _ in range(2)],#2x5x1024
                Conv2D(512, kernel_size=1, strides=2, use_bias=False),#1x3x2048
                [Res_Block(512, 512) for _ in range(2)],#1x3x2048
                GlobalAveragePooling2D(),#2048
                Dense(1000, activation='relu'),#1000
                Dense(output_dim, activation='relu')#1
        ]
